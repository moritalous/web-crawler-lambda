...
```bash
web-crawler-lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
web-crawler-lambda$ sam local start-api
web-crawler-lambda$ curl http://localhost:3000/
```
...
```bash
web-crawler-lambda$ sam logs -n HelloWorldFunction --stack-name web-crawler-lambda --tail
```
...
```bash
web-crawler-lambda$ cd hello-world
...
```bash
sam delete --stack-name web-crawler-lambda
```
